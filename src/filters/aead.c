...
bool aead_encrypt(AeadFilter *this, const Byte *plainText, size_t plainSize, Byte *header,
                  size_t headerSize, Byte *cipherText, size_t cipherSize,Byte *tag);
bool aead_decrypt(AeadFilter *this, Byte *plainText, size_t plainSize, Byte *header,
                  size_t headerSize, Byte *cipherText, size_t cipherSize, Byte *tag);
bool aeadCipherSetup(AeadFilter *this, char *cipherName);
bool aeadConfigure(AeadFilter *this);
 ...
 * @return - Error status.
 */
bool aeadFilterOpen(AeadFilter *this, const char *path, int oflags, int mode)
{

    /* Open the downstream file */
    int ret = fileOpen(nextStack(this), path, oflags, mode);
    if (ret < 0)
        return ret;
    ...
    this->fileSize = (oflags & O_TRUNC)? 0 : EOF;
    this->cipherBuf = malloc(this->encryptSize); /* TODO: memory allocation */
    this->plainBuf = malloc(this->decryptSize);
	this->cipher = NULL;
	this->ctx = NULL;
 ...
 */
ssize_t aeadFilterRead(AeadFilter *this, Byte *buf, size_t size, off_t offset, void *ctx)
{
	...
	size_t blockNr = (offset / this->decryptSize);
	off_t cipherOffset = blockNr * this->encryptSize;

    /* Read a block of downstream encrypted text into our buffer. */
    size_t actual = fileReadAll(nextStack(this), this->cipherBuf, this->encryptSize, cipherOffset, ctx);
    if (actual < 0)
        return actual;
    ...
    size_t plainSize = aead_decrypt(this, buf, size, NULL, 0, this->cipherBuf, cipherTextSize, tag); /* include block nr */
    if (plainSize < 0)
        return plainSize;

    /* Track our position for EOF handling */
    this->maxReadPosition = MAX(this->maxReadPosition, offset + plainSize);

	/* If a partial read, then we're at EOF and know the actual file size */
    if (plainSize < this->decryptSize)
        this->fileSize = this->maxReadPosition;

    /* If partial block, probe to make sure the file is really EOF. */
    if (plainSize < this->decryptSize)
    {
        ssize_t size = fileRead(nextStack(this), this->cipherBuf, 1, cipherOffset+actual, ctx);
        if (size < 0)
            return setIoStackError(this, "Encrypted file %s has extra data appended.", "name-not-implemented");
    }
    ...
    /* Track our position for EOF handling */
    this->maxReadPosition = MAX(this->maxReadPosition, this->blockNr * this->decryptSize + plainSize);
 ...
 */
size_t aeadFilterWrite(AeadFilter *this, const Byte *buf, size_t size, off_t offset, void *ctx)
{
    ...
    Byte tag[EVP_MAX_MD_SIZE];
    size_t plainSize = MIN(size, this->decryptSize);
    ...
    fileWriteAll(nextStack(this), this->cipherBuf, cipherSize, offset, ctx);

    /* Track our position for EOF handling */
    this->position += plainSize;
    this->maxWritePosition = MAX(this->maxWritePosition, this->position);
 ...
 */
bool aeadFilterClose(AeadFilter *this)
{
    ...
    /* CASE: we didn't write at the end, Don't write final record.*/
    if (this->maxReadPosition > this->maxWritePosition)
        ;

    /* CASE: the biggest write was a partial block. No need to add empty block */
    else if (this->maxWritePosition % this->decryptSize != 0)
        ;

    /* CASE: We know the file size and it is bigger than last write */
    else if (this->sizeConfirmed && this->fileSize > this->maxWritePosition)
        ;

    else
    {
        /* CASE: downstream file size is bigger than our highest write. (PADDING PROBLEMS!)*/
        off_t actualSize = fileSize(nextStack(this));
        off_t expectedSize = this->maxWritePosition / this->decryptSize * this->encryptSize + this->headerSize;
        if (actualSize > expectedSize)
            ;

        /* OTHERWISE: Final record is full. Write a zero length ending block */
        else
            fileWrite(this, NULL, 0, this->fileSize, this->ctx);
    }

    /* Notify the downstream file it must close as well. */
    fileClose(nextStack(this));

    /* TODO: free all resources includiong ctx and cipher. Maybe we should do before close in case it throws. */
 ...
 */
off_t aeadFilterSize(AeadFilter *this)
{
        /* Get the file size */
        size_t cipherSize = fileSize(nextStack(this));
        size_t dataSize = cipherSize - this->headerSize;

        /* Get offset of last blockcccccbbuknbrfjdlinrunfjrncthiffgdtgvdtvbvifj
         * */
